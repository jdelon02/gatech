...
/**
 * Provides a form for reverting a Event Instance revision.
 *
 * @ingroup bot_event_instance_entities
 */
class EventInstanceEntityRevisionRevertForm extends ConfirmFormBase {
  ...
  public function getFormId() {
    return 'event_instance_entity_revision_revert_confirm';
  }
  ...
  public function submitForm(array &$form, FormStateInterface $form_state) {
    ...
    $this->logger('content')->notice('Event Instance: reverted %title revision %revision.', ['%title' => $this->revision->label(), '%revision' => $this->revision->getRevisionId()]);
    $this->messenger()->addMessage(t('Event Instance %title has been reverted to the revision from %revision-date.', ['%title' => $this->revision->label(), '%revision-date' => $this->dateFormatter->format($original_revision_timestamp)]));
   ...
   *
   * @param \Drupal\bot_event_instance_entities\Entity\EventInstanceEntityInterface $revision
   ...
   */
  protected function prepareRevertedRevision(EventInstanceEntityInterface $revision, FormStateInterface $form_state) {
